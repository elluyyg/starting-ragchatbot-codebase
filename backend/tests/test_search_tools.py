...
class TestCourseSearchToolExecute:
    ...
    def test_execute_with_lesson_number_filters_correctly(self):
        ...
        mock_results = SearchResults(
            documents=[
                "Lesson 5 content: This is about MCP servers and how to build them."
            ],
            metadata=[{
        ...
        assert call_kwargs.get('lesson_number') == 5, \
    ...
    def test_execute_with_lesson_number_returns_lesson_5_content(self):
        ...
        assert "Lesson 5" in result or "lesson 5" in result.lower(), \
    ...
    def test_execute_without_lesson_number_returns_all_lessons(self):
        ...
        mock_results = SearchResults(
            ...
            metadata=[
                {"course_title": "MCP Course", "lesson_number": 1, "lesson_link": "https://example.com/1"},
                {"course_title": "MCP Course", "lesson_number": 2, "lesson_link": "https://example.com/2"},
                {"course_title": "MCP Course", "lesson_number": 5, "lesson_link": "https://example.com/5"}
            ],
        ...
        call_kwargs = mock_store.search.call_args.kwargs
        assert call_kwargs.get('lesson_number') is None, \
    ...
    def test_execute_tracks_sources_for_ui(self):
        ...
        assert first_source.get('lesson_number') == 5 or "5" in str(first_source), \
...
class TestCourseSearchToolSorting:
    ...
    def test_results_sorted_by_course_then_lesson_number(self):
        ...
        assert 'Building Towards' in headers[0], f"Expected Building Towards first, got: {headers[0]}"
        ...
        assert 'Prompt Compression' in headers[3], f"Expected Prompt Compression fourth, got: {headers[3]}"
    ...
    def test_sorting_uses_lesson_number_field_not_title(self):
        ...
        mock_results = SearchResults(
            ...
            metadata=[
                {'course_title': 'Course A', 'lesson_number': 10, 'lesson_link': 'link10'},  # lesson 10
                {'course_title': 'Course A', 'lesson_number': 2, 'lesson_link': 'link2'},    # lesson 2
            ],
            distances=[0.1, 0.2]
        )
        ...
        sources = tool.last_sources
        assert 'Lesson 2' in sources[0]['title'], f"Expected Lesson 2 first, got: {sources[0]['title']}"
        assert 'Lesson 10' in sources[1]['title'], f"Expected Lesson 10 second, got: {sources[1]['title']}"
...
class TestCourseSearchToolDefinition:
    ...
    def test_tool_definition_lesson_number_is_integer(self):
        ...
        assert lesson_number_schema.get('type') == 'integer', \
